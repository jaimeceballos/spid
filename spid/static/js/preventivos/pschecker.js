(function ($) {
    $.fn.extend({
        pschecker: function (options) {
            var settings = $.extend({ minlength: 6, maxlength: 12, onPasswordValidate: null, onPasswordMatch: null }, options);
                    ...
                    if (pstr.length > 0) {
                        ...
                        if (rx.test(pstr)) {
                            ...
                            if (result > 2) {
                                ...
                                meter.addClass('week');
                                meter.html("Contraseña Débil");
                            }
                        }
                        if (cPassword.val().toString().length > 0) {
                            if (pstr == cPassword.val().toString() & rx.test(pstr)) {
                                cPassword.removeClass('no-match');
